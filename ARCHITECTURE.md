...
`GET /api/portal/pass` devuelve las tarjetas ganadas únicamente para la identidad de la sesión. `PUT` rechaza equipar una tarjeta de recompensa sin una tirada registrada para ese alumno. El carrusel conserva las portadas anteriores y añade las desbloqueadas. Estas seis ilustraciones admiten holográfico de tarjeta completa, sin máscaras separadas de personaje y fondo. Su procedencia está en [design/gachapon](apps/xp-canvas/design/gachapon/README.md).
